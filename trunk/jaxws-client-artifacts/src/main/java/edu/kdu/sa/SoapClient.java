...
public class SoapClient {
	...
	public static void main(String[] args) {
		try {
			...
			// An alternate way to get the SOAP service interface; includes
			...
			LOGGER.debug(
					"Time required to initialize weather service interface: {} seconds",
					(end - start) / 1000f);

			// Send a SOAP weather request for zip code 94025 (Menlo Park, CA,
			// USA).
			LOGGER.debug("weatherSoap instance: {}", weatherSoap);
			start = new Date().getTime();
			ForecastReturn forecastReturn = weatherSoap
					.getCityForecastByZIP("94025");
			end = new Date().getTime();
			System.out.println("Test....");
			LOGGER.debug(
					"Time required to invoke 'getCityForecastByZIP': {} seconds",
					(end - start) / 1000f);
			...
			LOGGER.debug("forecastReturn result: {}",
					forecastReturn.getForecastResult());
			LOGGER.debug("forecastReturn response text: {}",
					forecastReturn.getResponseText());
			LOGGER.debug("");
			...
			for (Forecast forecast : forecasts) {
				LOGGER.debug(
						"  forecast date: {}",
						DATE_FORMAT.format(forecast.getDate()
								.toGregorianCalendar().getTime()));
				LOGGER.debug("  forecast description: {}",
						forecast.getDesciption());
				Temp temps = forecast.getTemperatures();
				LOGGER.debug("  forecast temperature high: {}",
						temps.getDaytimeHigh());
				LOGGER.debug("  forecast temperature low: {}",
						temps.getMorningLow());
				POP pop = forecast.getProbabilityOfPrecipiation();
				LOGGER.debug("  forecast precipitation day: {}%",
						pop.getDaytime());
				LOGGER.debug("  forecast precipitation night: {}%",
						pop.getNighttime());
				LOGGER.debug("");
			}
